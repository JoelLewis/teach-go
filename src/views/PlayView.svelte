<script lang="ts">
  ...
  onMount(() => {
    ...
    // A game starts immediately either way: KataGo when it's installed,
    // the built-in Practice Bot while the download is still in progress.
  ...
  async function startNewGame() {
    ...
    try {
      ...
      gameStore.set(state);
      aiEngine = await api.getAiEngine();
